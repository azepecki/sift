  ...
  let build_function_body fdecl =
    ...
    *)
    ...
    let rec build_expr table builder ((typ, e) : sexpr) = match e with
      ...
      | SFloatLit d  -> L.const_int float_t d
    ...
    let rec build_stmt table builder stmt = match stmt with
     ...
     | SIfElse (predicate, then_stmt, else_stmt) ->
      ...
      | SFor (declr, predicate, inc, body) -> 
        match body with
        | SBlock(lst) -> build_stmt table builder (SBlock([declr ; SWhile(predicate, SBlock(lst @ [SExpr(inc)]))]))
        | _           -> build_stmt table builder (SBlock([declr ; SWhile(predicate, SBlock(body :: [SExpr(inc)]))]))
