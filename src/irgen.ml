...
in
  ...
  let build_function_body fdecl =
    ...
    *)
    ...
    let rec build_stmt table builder loop stmt = 
     ...
     | SIfElse (predicate, then_stmt, else_stmt) ->
      ...
      | SWhile (predicate, body) ->
        ...
        let (new_builder, _, _) = build_stmt table (L.builder_at_end context body_bb) loop body in
        add_terminal (new_builder) build_br_while;

        let end_bb = L.append_block context "while_end" the_function in

        ignore(L.build_cond_br bool_val body_bb end_bb while_builder);
        (L.builder_at_end context end_bb, table, Some(while_bb, end_bb))
